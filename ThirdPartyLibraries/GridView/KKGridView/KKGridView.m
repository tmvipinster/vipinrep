...
- (void)_sharedInitialization
{
    ...
    // Set up defaults
    self.cellSize = CGSizeMake(90.0f, 90.0f);
...
- (void)_layoutExtremities
{
    ...
    if (_gridFooterView != nil) {
        ...
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
             ...
             footerRect.origin = (CGPoint) { .y = (self.contentSize.height - footerRect.size.height) + 15.0 };
        }
       
        footerRect.origin.x = (self.bounds.size.width - _gridFooterView.frame.size.width)/2.0 - 5;
        footerRect.size.width = _gridFooterView.frame.size.width;
...
- (KKGridViewCell *)_loadCellAtVisibleIndexPath:(KKIndexPath *)indexPath
{
    ...
    cell.isLongPressed = self.isLongPressed;
    return cell;
}
...
- (void)reloadCellAtIndexPath:(KKIndexPath*)path
{
    KKGridViewCell *cell = [_visibleCells objectForKey:path];
    if (cell) {
        cell.hidden = YES;
...
- (void)reloadItemsAtIndexPaths:(NSArray *)indexPaths
{
    for (KKIndexPath *path in indexPaths) {
        [self reloadCellAtIndexPath:path];
    }
}
...
- (void)reloadContentSize
{
    ...
    // NSLog(@"Grid view %f",self.bounds.size.width);
