...
-(BOOL) isSSLValidationRequired
{
    BOOL isValidationRequired = YES;
    
    if ([[NSBundle mainBundle] objectForInfoDictionaryKey:@"SSLValidationRequired"]){
        isValidationRequired = [[[NSBundle mainBundle] objectForInfoDictionaryKey:@"SSLValidationRequired"] boolValue];
    }
...
- (BOOL)connection:(NSURLConnection *)connection canAuthenticateAgainstProtectionSpace:(NSURLProtectionSpace *)protectionSpace;
{
    if ([[protectionSpace authenticationMethod] isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        return [self isSSLValidationRequired];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge;
{
    if([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
        ...
        if(trusted)
        {
            //Certificate is trusted, proceed normally.
    ...
    BOXLog(@"Error: Not handled %@", challenge.protectionSpace.authenticationMethod);
}
