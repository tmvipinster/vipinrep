...
-(id)init
{
    ...
    if(self)
    {
        ...
        // because the OAuth2 session enqueues API operations to fetch access tokens and the queue
        // manager uses the OAuth2 session as a lock object when enqueuing operations.
        self.queueManager = [[BoxParallelAPIQueueManager alloc] init];
        self.OAuth2Session = [[BoxParallelOAuth2Session alloc] initWithClientID:nil
                                                                         secret:nil
                                                                     APIBaseURL:BoxAPIBaseURL
                                                                   queueManager:self.queueManager];
        
        self.queueManager.OAuth2Session = self.OAuth2Session;
        
        self.filesManager = [[BoxFilesResourceManager alloc] initWithAPIBaseURL:BoxAPIBaseURL OAuth2Session:self.OAuth2Session queueManager:self.queueManager];
        self.filesManager.uploadBaseURL = BoxAPIUploadBaseURL;
        self.filesManager.uploadAPIVersion = BoxAPIUploadAPIVersion;
        
        self.foldersManager = [[BoxFoldersResourceManager alloc] initWithAPIBaseURL:BoxAPIBaseURL OAuth2Session:self.OAuth2Session queueManager:self.queueManager];
        
        self.usersManager = [[BoxUsersResourceManager alloc] initWithAPIBaseURL:BoxAPIBaseURL OAuth2Session:self.OAuth2Session queueManager:self.queueManager];
        self.searchManager = [[BoxSearchResourceManager alloc] initWithAPIBaseURL:BoxAPIBaseURL OAuth2Session:self.OAuth2Session queueManager:self.queueManager];
        self.commentsManager = [[BoxCommentsResourceManager alloc]initWithAPIBaseURL:BoxAPIBaseURL OAuth2Session:self.OAuth2Session queueManager:self.queueManager];
